...
## Equipment ceiling (`exercises.max_weight`, `is_active`)
Ben's adjustable dumbbells max at 80 lbs/hand (one pair owned). `exercises.max_weight` (NUMERIC, default 80, nullable = no cap) enforces this at the DB level; `runProgressionEngine` reads it and passes `Number(ex.max_weight)` (Postgres returns numeric columns as strings via supabase-js — always coerce) as `opts.maxWeight` into `computeProgressionStep`. **This is generic capping machinery only — no per-exercise progression ladders beyond the ceiling are designed or built.** Do not hand-author what happens after an exercise maxes out; that is `'maxed'`, a terminal state (see below).

Behavior inside `computeProgressionStep` when `maxWeight` is set (all no-ops when `null`, so uncapped exercises and all of rehab are byte-for-byte unaffected):
- **Set 3 clamp:** every place set3 would advance by 5 lbs (`ready`-state classic advance, rep-ladder ceiling-hit advance) clamps to `Math.min(s3w + 5, maxWeight)`. Set 3 never proposes a weight above the ceiling.
- **Flat ramp at the ceiling:** once `s3w >= maxWeight` (`atCap`), the percentage ramp (`classicTargets`) is replaced by flat targets `{ target_s2: maxWeight, target_s1: maxWeight }` for both catch-up states — a percentage of a set-3 weight that can no longer move stops making sense as a target. This flattening pass fires for **any** variant once capped (not just `rep_ladder`), via the same `'ready'`-state catch-up correction that handles rep-ladder freshly-seeded lag (see above) — `atCap` is OR'd into that condition.
- **Rep ceiling once fully flat:** once all three sets sit at `maxWeight` (`allAtCap`), a `'ready'`-state evaluation switches to rep-ceiling logic regardless of variant: hit `goalReps + 5` on set 3 with confirm → **terminal** `'maxed'`, not a weight bump (there's nowhere left to add weight). Below ceiling but ≥ goal → stay `'ready'`, no-op.
- **`'maxed'` is terminal, no auto-swap.** `computeProgressionStep` returns `{}` immediately whenever `progressionState === 'maxed'` — stall counting, catch-up, everything stops. The only way out is a manual correction (see below); nothing in the engine ever un-maxes an exercise on its own.
- **Gate ordering is unaffected.** All of this lives inside `computeProgressionStep`; the existing gates (ROM hold, deload freeze, amber suppression) still run in `runProgressionEngine` *before* it decides whether/how to call `computeProgressionStep` at all — the cap cannot bypass them.

**UI:** `progressionHint()` returns the ceiling-flattened text once `atCap` (before falling through to the normal rep-ladder/classic branches), and `''` once `'maxed'` (the persistent card badge covers it — see below). `rExerciseRow` shows a persistent "Maxed out — needs a new progression plan." badge whenever `st.progression_state === 'maxed'`, unconditional on active/done/pending — it's terminal info, not a workout-in-progress hint. `rLogSet`'s set-3 confirm block shows the same static message instead of the confirm button when maxed. `defaultReps()` and the rep-ladder reminder on sets 1–2 both also treat `allAtCap` as rep-ceiling mode so the set-3 pre-fill and reminder text stay consistent with the engine's evaluation, even for exercises whose `progressionVariant` would otherwise say `'classic'` (any exercise near the 80 lb ceiling is well past the 20% rep-ladder threshold, so this only matters once capped).

`PROG_STATES` (the manual weight-editor's state-cycle button, long-press state panel → exercise edit — see `.claude/rules/frontend.md`) includes `'maxed'` so it can be manually set or cleared — the only way out of the terminal state today.

**`is_active`** (BOOLEAN, default true) excludes an exercise from `loadExercises()`'s query (`.eq('is_active', true)`) — it stops appearing in workout generation but its `exercise_state` and `session_sets` history are untouched and still read correctly. Used to retire an exercise without deleting or renaming it (see Glute bridge → single-leg swap below) — renaming or deleting would corrupt how past sessions display.

### Glute bridge → single-leg glute bridge (2026-08-22)
Bilateral Glute bridge (`a3000003`) was deactivated (`is_active = false`), not renamed or deleted, so its `session_sets` history keeps reading as the bilateral exercise it was actually logged as. `a3000006` ("Single-leg glute bridge") replaces it at the same `gym_day`/`day_order` (3/3 — no uniqueness constraint on that pair, reuse is fine), seeded light at 20/20/20 `'ready'` deliberately — Ben already had reliable form and wanted the engine to climb rather than start heavy. It is **not** knee-loading (hip extension, negligible patellofemoral load) — not added to `KNEE_LOADING_EXERCISES`/`isKneeLoading`, no deload freeze, no ROM/phase gate (`progression_hold_until_phase` left `NULL`). It follows the same "log 3 sets, no separate per-leg mechanic" convention as Single-leg RDL and Single-leg bench squat — no new unilateral-logging code was introduced.
...
### Equipment ceiling: 80 lbs, and no ladder beyond it
**Rule:** `exercises.max_weight` defaults to 80 (Ben's adjustable dumbbells, one pair). Once capped, the flat-ramp/rep-ceiling machinery in `computeProgressionStep` takes over (see Equipment ceiling above); once fully maxed, `'maxed'` is terminal with no auto-swap.
**Evidence:** **Hardware constraint, not clinical or training-theory derived.** 80 lbs/hand is the physical maximum the owned equipment supports — this is not a deliberate training ceiling, just what's in the house. Per-exercise plans for what to do once an exercise maxes out (switch to barbell work, add a second pair, change the movement pattern entirely) are **deliberately undesigned** — Ben asked explicitly not to invent progression ladders for any specific exercise, since almost none of them are within years of 80 lbs and speculative design now would be guessing at a problem that doesn't exist yet. When an exercise actually hits `'maxed'`, that's the trigger to have the real conversation with Ben about what comes next for that specific movement — not a signal to guess at one in code.
